...
| Module | Role |
|--------|------|
| `app/admin_app.py` | `app_env` + FastAPI routes (`/`, `/auth/login`, `/auth/callback`, `/auth/logout`, `/workspace/enable`, `/workspace/create`, `/launch`, `/launch/status`, `/stop`, `/health`). `/workspace/enable` is the opt-in that forks the upstream repo and records the verified `fork_full_name`; `/workspace/create` writes a new notebook to the fork's `main` and returns the rendered tile; `/launch` serves a per-notebook env (workspace launches require opt-in); `/launch/status` reports the user's active per-notebook apps (Flyte-queried) so the dashboard hydrates running tiles to Open/Stop; `/stop` deactivates by name (`App.get(...).deactivate()`). Lifespan runs `init()` at startup. |
