...
| `app/notebook_meta.py` | Static parsing of a notebook's PEP 723 `[tool.stargazer]` resource block (no code execution, honored as-authored — no ceiling); plus the header-rewrite helpers used by `/workspace/create`: `with_stargazer_resources` (resources) and `with_pinned_marimo` (pins the sandbox kernel's marimo to the launcher's version). |
