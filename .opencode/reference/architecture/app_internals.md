...
- **Layout** is a short static force simulation (`layout()`: charge repulsion + edge springs + center gravity, fixed iterations, drawn once — no animation loop competing with the starfield). Nodes are capped at `NODE_CAP` (150) with a "showing N of M" hint past the cap so a large account can't hang the page.
- **Interaction:** hover a node peeks a styled tooltip (`graph-tip`) and highlights its edges/neighbors; click pins the **detail card** (full metadata, system `_*` keys dimmed, linked-asset chips, a Download button → `/assets/download/{cid}?network=`). List rows share the same `selectAsset()` path. There is no edit/delete (plan 20 out-of-scope; MCP is the escape hatch).
