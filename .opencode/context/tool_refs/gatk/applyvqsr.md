...
Running:
    java -Dsamjdk.use_async_io_read_samtools=false -Dsamjdk.use_async_io_write_samtools=true -Dsamjdk.use_async_io_write_tribble=false -Dsamjdk.compression_level=2 -jar /opt/gatk/gatk-package-4.6.1.0-local.jar ApplyVQSR --help
...
--output,-O         The output filtered and recalibrated VCF file in which each variant is annotated with its
                              VQSLOD value  Required. 

--recal-file    The input recal file used by ApplyVQSR  Required. 
...
--exclude-filtered   Don't output filtered loci after applying the recalibration  Default value: false.
                              Possible values: {true, false} 
...
--ignore-filter       If specified, the recalibration will be applied to variants marked as filtered by the
...
--mode                  Recalibration mode to employ: 1.) SNP for recalibrating only SNPs (emitting indels
...
--tranches-file     The input tranches file describing where to cut the data  Default value: null. 
...
--use-allele-specific-annotations,-AS 
                              If specified, the tool will attempt to apply a filter to each allele based on the input
...
--lod-score-cutoff    The VQSLOD score below which to start filtering  Default value: null. 
